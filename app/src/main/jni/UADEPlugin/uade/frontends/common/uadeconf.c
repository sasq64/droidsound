...
int uade_set_song_attributes(struct uade_state *state, char *playername, size_t playernamelen)
{
...
#define SET_OPTION(opt, value) do { uc->opt = (value); uc->opt##_set = 1; } while (0)
    ...
    case UC_BUFFER_TIME:
        if (value != NULL) {
            ...
            __android_log_print(ANDROID_LOG_VERBOSE, "UADE", "UADE: Passed NULL to UC_BUFFER_TIME.\n");
        }
        break;
    ...
    case UC_EAGLEPLAYER_OPTION:
        if (value != NULL) {
            ...
            __android_log_print(ANDROID_LOG_VERBOSE, "UADE", "UADE: Passed NULL to UC_EAGLEPLAYER_OPTION.\n");
        }
        break;
    ...
    case UC_SUBSONG_TIMEOUT_VALUE:
        if (value == NULL) {
            __android_log_print(ANDROID_LOG_VERBOSE, "UADE", "UADE: UC_SUBSONG_TIMEOUT_VALUE is NULL\n");
            break;
        }
