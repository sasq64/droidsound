...
public final class Player implements Runnable {
	...
	public Player(AudioManager am, Handler handler, Context ctx) {
		...
		// samples = new short[bufSize / 2]; // >> is more efficient than
		// dividing by powers.
	...
	public static String makeSize(long fileSize) {
		...
		if (fileSize < 10 << 10) {
			...
			// s = String.format("%dKB", fileSize / 1024); // >> is more
			// efficient for powers.
			...
			// s = String.format("%dMB, fileSize / (1024 * 1024)); Simplified
			// using a shift.
	...
	private void dumpWav(SongFile song, File outFile, int length, int flags)
			throws IOException {
		...
		// int dataSize = wavSamples * numChannels * 16/8; Simplified using a
		// shift.
		int dataSize = (wavSamples * numChannels << 4) >> 3;

		Log.d(TAG, "%dsec => %d samples, %d wavSamples, %d dataSize",
				length / 1000, numSamples, wavSamples, dataSize);
		...
		while (sampleCount < numSamples) {
			// In and out are number of shorts, not number of samples (2 bytes
			// vs 4 bytes in 16bit stereo)
			int len = currentPlugin.getSoundData(samples, bufSize / 16);
			Log.d(TAG, "READ %d (%d)", len, bufSize);
			if (len < 0) {
				wavSamples = sampleCount / divider;
				// dataSize = wavSamples * numChannels * 16/8; Simplified using
				// a shift.
			...
			int avg = ((int) samples[i] + (int) samples[i + 1]
		...
		Message msg = mHandler.obtainMessage(MSG_WAVDUMPED, 0, 0,
				outFile.getPath());
		mHandler.sendMessage(msg);
	}

	/**
	 * @param song		The actual track file
	 * @param skipStart
	 */
	private void startSong(SongFile song, boolean skipStart) {
		...
		// List<DroidSoundPlugin> list = new ArrayList(); //Below is weaker type
		// of same thing
		...
		try {
			...
			if (song.getZipPath() != null) {
				...
				fileSize = songFile.length();
				// System.gc(); generally unecessary. Garbage Collection is
				// automated
		...
		if (songBuffer != null || songFile != null || streamName != null) {
			...
			if (currentPlugin == null) {
				for (DroidSoundPlugin plugin : plugins) {
					if (!plugin.canHandle(song.getName())) {
						/*
						 * if(streamName != null) { try { songLoaded =
						 * plugin.loadStream(streamName); fileSize =
						 * plugin.getStreamSize(); } catch (IOException e) {
						 ...
						 */if (songFile != null) {
							...
							songLoaded = plugin.load(baseName, songBuffer,
									(int) fileSize);
							if (songLoaded)
								Log.d(TAG, "%s gave Songref %s", plugin
										.getClass().getName(),
										songLoaded ? "TRUE" : "FALSE");
							if (songLoaded) {
								currentPlugin = plugin;
								break;
							}
			...
			if (currentPlugin != null) {
				...
				synchronized (this) {
					...
					// currentSong.game =
					// getPluginInfo(DroidSoundPlugin.INFO_GAME);
					 ...
					 */
					System.arraycopy(info, 0, currentSong.details, 0,
							info.length); // Improved code, eliminating the for
											// loop
					...
					if (currentSong.title == null
							|| currentSong.title.equals("")) {

						if (currentPlugin.delayedInfo()) {
							currentSong.title = null;
						} else {
							String basename = currentPlugin
									.getBaseName(currentSong.fileName);
				...
				if (!skipStart) {
					...
					Log.d(TAG, ":%s:%s:%s:%s:", currentSong.title,
					...
					if (reinitAudio) {
						...
						audioTrack = new AudioTrack(AudioManager.STREAM_MUSIC,
								FREQ, AudioFormat.CHANNEL_CONFIGURATION_STEREO,
								AudioFormat.ENCODING_PCM_16BIT, bufSize,
								AudioTrack.MODE_STREAM);
						samples = new short[bufSize / 2];
						reinitAudio = false;
				...
				lastPos = -1000;
				if (songFile2 != null) {
					Log.d(TAG, "Deleting temporary files %s and %s",
	...
	public void run() {
		...
		reinitAudio = false;
		Log.d(TAG, "AudioTrack created in thread "
		...
		while (noPlayWait < 50) {
			try {
				if (command != Command.NO_COMMAND) {
					...
					Log.d(TAG, "Command %s while in state %s",
							command.toString(), currentState.toString());
					...
					synchronized (cmdLock) {
						...
						case STOP:
							if (currentState != State.STOPPED) {
								// audioTrack.pause();
								Log.d(TAG, "STOP");
								MediaPlayer mp = currentPlugin == null ? null
										: currentPlugin.getMediaPlayer();
								if (mp != null) {
									mp.stop();
								} else {
								...
								currentPlugin = null;
								currentState = State.STOPPED;
								Message msg = mHandler.obtainMessage(MSG_STATE,
										0, 0);
								mHandler.sendMessage(msg);
							}
							break;
						...
						if (currentState != State.STOPPED) {
							...
							case SET_POS:
								...
								if (currentPlugin.seekTo(msec)) {
									...
									//
									int playPos = audioTrack
											.getPlaybackHeadPosition()
											* 10
											/ (FREQ / 100);
									Log.d(TAG, "Offset %d/1000 - %d", msec,
											playPos);
							...
							case SET_TUNE:
								...
								if (currentPlugin.setTune((Integer) argument)) {
									...
									currentSong.length = currentPlugin
											.getIntInfo(DroidSoundPlugin.INFO_LENGTH);
									currentSong.subtuneTitle = currentPlugin
											.getStringInfo(DroidSoundPlugin.INFO_SUBTUNE_TITLE);
									currentSong.subtuneAuthor = getPluginInfo(DroidSoundPlugin.INFO_SUBTUNE_AUTHOR);
									Message msg = mHandler
											.obtainMessage(
													...
													currentSong.length,
													new String[] {
															currentSong.subtuneTitle,
															currentSong.subtuneAuthor });
									mHandler.sendMessage(msg);
									audioTrack.play();
								}
								break;
				...
				if (currentState == State.PLAYING) {
					...
					if (mp != null) {
						...
						if (playPos > 0 && firstData) {
							...
							if (currentSong.title == null
								...
								int sep = basename.indexOf(" - ");
								if (sep > 0) {
									currentSong.author = basename.substring(0,
											sep);
									currentSong.title = basename
											.substring(sep + 3);
								} else
								...
								Message msg = mHandler.obtainMessage(MSG_INFO,
										0, 0, new String[] { currentSong.title,
												currentSong.author });
								mHandler.sendMessage(msg);
							}
						...
						if (currentPlugin
								.getIntInfo(DroidSoundPlugin.INFO_DETAILS_CHANGED) != 0) {

							String title = currentPlugin
									.getStringInfo(DroidSoundPlugin.INFO_TITLE);
							String author = currentPlugin
									.getStringInfo(DroidSoundPlugin.INFO_AUTHOR);

							String subTuneTitle = currentPlugin
									.getStringInfo(DroidSoundPlugin.INFO_SUBTUNE_TITLE);
							String subTuneAuthor = currentPlugin
									.getStringInfo(DroidSoundPlugin.INFO_SUBTUNE_AUTHOR);
							int length = currentPlugin
									.getIntInfo(DroidSoundPlugin.INFO_LENGTH);
							...
							if (subTuneTitle != null
									&& (!subTuneTitle
											.equals(currentSong.subtuneTitle))) {
								msg = mHandler.obtainMessage(MSG_SUBTUNE, -1,
										length, new String[] { subTuneTitle,
												subTuneAuthor });
							...
							if (title != null
									&& (!title.equals(currentSong.title))) {
								msg = mHandler.obtainMessage(MSG_INFO, 0, 0,
						...
						int song = currentPlugin
								.getIntInfo(DroidSoundPlugin.INFO_SUBTUNE_NO);
						if (song >= 0 && song != currentTune) {
							...
							currentSong.subtuneTitle = currentPlugin
									.getStringInfo(DroidSoundPlugin.INFO_SUBTUNE_TITLE);
							currentSong.subtuneAuthor = getPluginInfo(DroidSoundPlugin.INFO_SUBTUNE_AUTHOR);
							Message msg = mHandler.obtainMessage(MSG_SUBTUNE,
									currentTune, currentSong.length,
									new String[] { currentSong.subtuneTitle,
											currentSong.subtuneAuthor });
							mHandler.sendMessage(msg);
						}
					...
					if (currentState == State.PAUSED
							&& currentPlugin.getMediaPlayer() != null) {
						int latency = currentPlugin.getIntInfo(203);
						if (latency >= 0) {
	...
	final synchronized int getPosition() {
	...
	public final void paused(boolean pause) {
		synchronized (cmdLock) {
			if (pause) {
				Log.d(TAG, "Pausing");
	...
	public final void playMod(SongFile mod) {
		synchronized (cmdLock) {
	...
	public final void playMod(SongFile mod, SongFile nextMod) {
		synchronized (cmdLock) {
	...
	public final void setSubSong(int song) {
		synchronized (cmdLock) {
	...
	public final void setBufSize(int bs) {
		synchronized (cmdLock) {
			if (bufSize != bs) {
				bufSize = bs;
	...
	public final void dumpWav(SongFile song, String destFile, int length,
			int flags) {
		synchronized (cmdLock) {
